...
type HabitOptions = { iconOverride?:string; personalTargetOverride?:string; unitOverride?:Unit; scheduleDays?:number[] };
type LibraryData = { tasks:Task[]; owned:{id:string;taskId:string;isPaused:boolean;isArchived:boolean;scheduleDays:unknown;iconOverride?:string|null;personalTargetOverride?:number|null;unitOverride?:Unit|null}[]; stacks:{id:string;name:string;description:string;tasks:{task:Task}[]}[] };
...
export default function Library() {
  ...
  async function add(taskId:string, override?:HabitOptions) {
    ...
    const response=await fetch('/api/habits',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({taskId,iconOverride:selected.iconOverride||undefined,personalTargetOverride,unitOverride:selected.unitOverride,scheduleDays:selected.scheduleDays})});
  ...
  return <main className="shell">
    ...
    <div className="neo-stagger mt-6 grid gap-3">{items.map(task=>{
      const owned=data.owned.find(entry=>entry.taskId===task.id);const present=Boolean(owned&&!owned.isArchived&&!owned.isPaused);const archived=Boolean(owned?.isArchived);const isCustomizing=customizingId===task.id;const savedDays=Array.isArray(owned?.scheduleDays)?owned.scheduleDays as number[]:[0,1,2,3,4,5,6];const selected=options[task.id]??{scheduleDays:savedDays};const displayIcon=selected.iconOverride??owned?.iconOverride??task.icon;
      return <Card className="p-4" key={task.id}><div className="flex gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-[rgb(var(--primary)/.10)] text-2xl" aria-hidden="true">{displayIcon}</div><div className="min-w-0 flex-1"><Badge>{task.category.replace('_',' ')} · {task.difficultyTag}</Badge><h2 className="mt-2 font-extrabold">{task.name}</h2><p className="mt-1 text-sm leading-6 text-[rgb(var(--muted))]"><Sparkles className="mr-1 inline h-3.5 w-3.5 text-[rgb(var(--accent))]"/>{task.whyThisHabit}</p></div></div>
        <div className="mt-4 flex flex-wrap gap-2">{!archived&&<Button variant="secondary" size="sm" type="button" aria-expanded={isCustomizing} onClick={()=>setCustomizingId(isCustomizing?null:task.id)}><Settings2 className="h-4 w-4"/>Customize<ChevronDown className={`h-3.5 w-3.5 transition-transform ${isCustomizing?'rotate-180':''}`}/></Button>}{archived?<Button size="sm" type="button" onClick={()=>void restore(owned!.id)}><Plus className="h-4 w-4"/>Restore habit</Button>:<Button size="sm" type="button" onClick={()=>void add(task.id)}>{present?<><Check className="h-4 w-4"/>Save changes</>:<><Plus className="h-4 w-4"/>Add habit</>}</Button>}</div>
        {isCustomizing&&<fieldset className="mt-4 border-t-2 border-border pt-4"><legend className="sr-only">Customize {task.name}</legend><p className="text-xs font-black uppercase tracking-wide text-[rgb(var(--muted))]">Choose an icon</p><div className="mt-2 grid grid-cols-6 gap-2" role="radiogroup" aria-label={`Icon for ${task.name}`}>{EMOJIS.map(emoji=><button key={emoji} type="button" role="radio" aria-checked={displayIcon===emoji} aria-label={`Use ${emoji} for ${task.name}`} onClick={()=>updateOptions(task.id,{iconOverride:emoji})} className={`grid aspect-square place-items-center border-2 text-lg shadow-[2px_2px_0_rgb(var(--shadow))] transition active:translate-x-[2px] active:translate-y-[2px] active:shadow-none ${displayIcon===emoji?'border-border bg-[rgb(var(--accent))]':'border-border bg-[rgb(var(--surface))]'}`}>{emoji}</button>)}</div>
          ...
          <div className="mt-4"><p className="text-xs font-black uppercase tracking-wide text-[rgb(var(--muted))]">Active days</p><div className="mt-2 grid grid-cols-7 gap-1">{['S','M','T','W','T','F','S'].map((label,day)=><button key={`${label}-${day}`} type="button" aria-label={`Schedule ${['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][day]}`} aria-pressed={(selected.scheduleDays??[]).includes(day)} onClick={()=>{const days=selected.scheduleDays??[];updateOptions(task.id,{scheduleDays:days.includes(day)?days.filter(value=>value!==day):[...days,day].sort((a,b)=>a-b)})}} className={`grid aspect-square place-items-center border-2 border-border text-xs font-black ${(selected.scheduleDays??[]).includes(day)?'bg-accent text-foreground':'bg-surface text-muted-foreground'}`}>{label}</button>)}</div></div><p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">Your target replaces level scaling until you clear it. Scheduled habits only count on their selected days.</p></fieldset>}
      </Card>;
