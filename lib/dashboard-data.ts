...
export type ChallengeProgress = { health:boolean; mental:boolean; completed:number; total:number };
...
export async function dashboardData(userId:string) {
  ...
  const active=await prisma.userTask.findMany({where:{userId,isPaused:false,isArchived:false},include:{task:true},orderBy:{sortOrder:'asc'}});
  const scheduled = active.filter(habit => Array.isArray(habit.scheduleDays) ? (habit.scheduleDays as unknown[]).some(day => day === weekday) : true)
  ...
  const tasks:DashboardTask[]=scheduled.map(({id,task,iconOverride,personalTargetOverride,unitOverride,currentTarget,targetRegressionLog})=>{
  ...
  const streakStart=todayDate(user.timezone); streakStart.setUTCDate(streakStart.getUTCDate()-365);
  const allDays=await prisma.dailyLog.findMany({where:{userId,date:{gte:streakStart}},select:{date:true}});
  ...
  const challenge = await prisma.monthlyChallenge.upsert({where:{userId_monthKey:{userId,monthKey}},update:{},create:{userId,monthKey,title:'Monthly Momentum',description:'Complete one health win and one focus win from your routine on the same day — then take a moment to notice the difference.',exp:120,requirements:{health:true,mental:true}}})
  const challengeLogs = await prisma.dailyLog.findMany({where:{userId,date:todayDate(user.timezone)},include:{task:{select:{category:true}}}})
  const challengeProgress:ChallengeProgress = {health:challengeLogs.some(log=>log.task.category==='HEALTH'),mental:challengeLogs.some(log=>log.task.category==='MENTAL'),completed:0,total:2}
  challengeProgress.completed = Number(challengeProgress.health) + Number(challengeProgress.mental)
  ...
  return {user:{displayName:user.displayName,level:user.level,title:levelTitle(user.level),totalExp:user.totalExp,nextExp:expToNextLevel(user.level),darkMode:user.darkMode},tasks,done:tasks.filter(t=>t.done).length,growth,streak,welcomeBack,weeklyRecap,challenge:{...challenge,progress:challengeProgress},discoverUnlocked:user.hasCompletedFirstDay||daysAway>=1};
}
...
async function evaluateMisses(userId:string, timezone:string, createdAt:Date, level:number) {
  ...
  const firstDay = new Date(`${isoDay(createdAt, timezone)}T00:00:00.000Z`);
  if (yesterday.getTime() < firstDay.getTime()) return;
  const habits = await prisma.userTask.findMany({ where:{ userId, isPaused:false, task:{ type:'PROGRESSIVE' } }, include:{ task:true } });
  for (const habit of habits) {
    const start = habit.lastMissEvaluatedDate ? new Date(habit.lastMissEvaluatedDate) : new Date(firstDay);
    if (habit.lastMissEvaluatedDate) start.setUTCDate(start.getUTCDate() + 1);
    let misses = habit.consecutiveMisses;
    let current = habit.currentTarget ?? progressiveTarget(habit.task.baseTarget ?? 1, level, habit.task.scalingFactor ?? 0, habit.personalTargetOverride);
    let regressionLog = Array.isArray(habit.targetRegressionLog) ? habit.targetRegressionLog : [];
    for (const cursor = new Date(start); cursor <= yesterday; cursor.setUTCDate(cursor.getUTCDate() + 1)) {
      const scheduledDay = cursor.getUTCDay();
      if (Array.isArray(habit.scheduleDays) && !(habit.scheduleDays as unknown[]).some(day => day === scheduledDay)) continue;
      const date = new Date(cursor);
      const completed = await prisma.dailyLog.findUnique({ where:{ userId_taskId_date:{ userId, taskId:habit.taskId, date } }, select:{ targetAtCompletion:true } });
      const expected = habit.personalTargetOverride ?? current;
      if (completed && (completed.targetAtCompletion ?? 0) >= expected) {
        misses = 0;
      } else {
        misses += 1;
        const base = habit.task.baseTarget ?? 1;
        if (misses >= 3 && !habit.personalTargetOverride) {
          const nextTarget = regressTarget(current, base);
          if (nextTarget < current) regressionLog = [...regressionLog, { date:isoDay(date, timezone), oldTarget:current, newTarget:nextTarget }];
          current = nextTarget;
          misses = 0;
        }
      }
      await prisma.userTask.update({ where:{ id:habit.id }, data:{ consecutiveMisses:misses, currentTarget:current, targetRegressionLog:regressionLog, lastMissEvaluatedDate:date } });
    }
  }
}
